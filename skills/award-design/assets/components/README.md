...
## Composition (`../../references/<archetype>.md`)

How past winners composed a page with these pieces lives in each archetype's tier-2 reference, loaded by heading at the design_plan commit: **Page recipe** carries the macrostructure anatomy, the hero architectures, the section chain (role · form · pairs · intensity · the state each section owes), the footer, the arrival, the mobile answer and the variation clause; **Spectacle menu** carries the hero beat, the continuation beats and the peak law the build's conformance table diffs against; **Effect palette** carries the per-element-class recipes with their tap, focus and dormancy answers. Each file closes on a generated **Component index** — every id here that fits the archetype, with the opening of its `whenToUse`. Read a row there, then grep this manifest for that id's contract. The composition is corpus evidence, never a script: steal the shapes, diverge with reasons.
