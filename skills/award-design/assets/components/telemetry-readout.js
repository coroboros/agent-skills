 ...
 * campaign's own record, not a winner citation — one dead build's
 * [data-hud-alt] / [data-hud-phase] altitude + phase readers with piecewise
 * value maps, a second's descent HUD — the 2400M altitude countdown and the
 * 'acquiring a fix' coordinate resolve where digits stand masked as '·' until
 * progress earns them, a third's trace rail — the vermilion thread filling
 * with descent and a 'SIGNAL n%' read riding it. Three dead builds each
 * re-invented this HUD with zero library coverage — recurring invention across
 * builds is what promotes a pattern into the library). Scroll progress through
 * a tracked span drives builder-authored readout channels; the component only
 * writes textContent and transforms — it never invents chrome, copy, or units.
 *
 ...
 *                       piecewise-linear keyframe map progress→value (the
 *                       first build's altFor grammar: any segment count,
 *                       plateaus included); shorthand
 *                       data-tel-from/data-tel-to = a two-key map.
 ...
 *                       acquiring a fix, not noise (the second build's
 *                       resolveCoord).
 ...
 *                       along the rail with progress (the third build's trace
 *                       read — transform-only; the corpse wrote style.height/top,
 ...
 * taller than the viewport (the second build's pin formula; author the root as
 * the tall tracked section). data-tel-span="page" reads whole-document progress
 * instead (the first build's grammar). One passive scroll listener per
 * instance, read once per rAF frame; an IntersectionObserver parks the work
 * while the root is off-screen.
 *
...
(function (global) {
  ...
  function injectCss() {
    ...
    s.textContent =
      ...
      // overflow:hidden here ate the riding 'SIGNAL n%' read whole).
  ...
  function formatValue(v, format) {
    ...
    return Math.round(v).toLocaleString('en-US'); // 'int' — the corpse fmt
  }
  ...
  function progressOf(inst) {
    ...
    if (scrollable <= 0 || !Number.isFinite(scrollable)) return 0; // fail-safe (corpse-derived)
