...
- A single-token change (one color, one radius) → `/design-system`; everyday work with no award ask → `/frontend-dev` when installed. Backend, data, infra → never.
