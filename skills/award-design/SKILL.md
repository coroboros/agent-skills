...
**Load now:** `references/external-truth.md` + `references/imagery.md` + `references/award-imperatives.md` + `references/motion-palette.md`.
...
- **Motion is motivated or absent — and scroll-linked when it moves.** Every animation answers "what does this communicate" in one sentence (hierarchy, storytelling, feedback, state); unable to ship it working → drop the Motion dial and ship clean static rather than half-built choreography. The default model is **reversible and scroll-linked** (`references/motion-palette.md`): on-scroll animation unrolls and re-rolls with the scroll — native `animation-timeline: view()/scroll()` is the primitive — never a fire-once IntersectionObserver reveal that dies after one pass (the IO path is the re-arming fallback). The archetype's restraint sets how *much* moves; the model sets *how*.
