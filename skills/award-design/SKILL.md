...
- **Commit** explicit per-element selections: hero architecture, the **navigation pattern** (show-on-scroll-up header or full-screen overlay — never "no nav"), type stack, color roles, the real visual per section, motion paradigms, the **signature interaction** (named, unforgettable, not a load fade), spacing rhythm, and the locked craft layer (named, not implied) — citing the Phase 0 dials where they bind (Density → spacing rhythm, Variance → grid asymmetry, Motion → the pacing ceiling), so the declared dials are spent, not decorative. The transverse imperatives (`references/award-imperatives.md`) are committed here, not discovered at ship. **Declare the interaction ingredient-set** — the one coherent hover/reveal/text-emphasis vocabulary applied to every interactive element (`references/interaction-signatures.md`, `references/text-effects.md`), plus the two-or-three section-tied signature echoes that keep the world alive past the hero; the substrate is committed like the type stack, not left to chance.
