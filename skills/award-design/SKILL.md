...
3. Fix, re-render, loop. Exit only when both widths pass in the *same* iteration. Cap: 5 loops per section — drift still standing at the cap is filed in the Phase 5 verdict, never silently accepted. Each section's exit writes a **ledger row** — section · iterations run · widths passed · capture references · defects found→fixed; the per-section proof is this ledger, never a narration. Every section exiting clean at iteration 1 on both widths is a uniform-verdict anomaly — note it in the Phase 5 verdict (first-try uniformity is evidence about the gate, not the build).
