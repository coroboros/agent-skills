...
The one instrument that judges quality. It runs ONCE, late, on the rendered site, after the mechanical layers are clean — a judge run over a broken page returns unusable signal on everything else. Its report IS the verdict artifact: the builder never writes READY, and no phase emits a ship status before this review returns.
...
- **Inventory before anchoring.** The reviewer describes the rendered page in its own words — sections, media, what moves — BEFORE reading the direction contract, the DESIGN.md, or any mechanical report. The contract is the builder's abstraction; a review anchored on it inherits whatever that abstraction dropped.
