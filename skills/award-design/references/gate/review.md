...
2. **The density read follows the selected archetype:** compare its committed medium and motion craft with the relevant exemplar. Minimalist may omit ambient idle; immersive/experimental sustain their live medium; a brutalist arc may defer spectacle to the footer. Judge missing declared choreography or a generic static substitute as a `concept` cause, never enforce another archetype's channel count.
