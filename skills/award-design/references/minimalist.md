...
Sources for the claims below are listed in the public research corpus of `github.com/coroboros/research`, under `articles/award-winning-websites-2025-2030/`: `archetypes/minimalist.md`.
...
**Anatomy** — *Product-narrative scroll* (`argument-scroll`; Terminal, winner-verified order; Treize softer): hero statement · attention → logo strip · proof → stat odometer · understanding → "That's the Yard Operating System. YOS™" reveal · climax, mid-page → three benefits · understanding → executive quote · proof → contact + oversized-wordmark footer · close; rest is the void gap, seams masked, never hard cuts. *Gallery-index stack* (`gallery-stack`; Gabriel, Stefan, winner-verified): a scramble-decode type intro assembling over a MAIN GALLERY of figures on the homepage · attention → one full-bleed project per viewport, masked inverse-scale reveals · proof (the first still below the fold is the climax) → home↔detail route transition as the cross-view beat → bare "SCROLL UP" close. *Rendered-medium scroll* (scene scroll (no catalog slug of its own yet); Terminal's `<canvas>` frame-sequence hero, Pacome's Three.js showreel): the hero carries the loud climax at intensity 9 with a scroll-scrubbed frame sequence as its medium — a composed static first frame, the dissolve on scroll — then a proof strip, an operable type-tester beat and the odometer band carry the mid-page over a void-gap rest. Restraint holds in the type and the palette; the richness is the medium and one operable beat. Reach here when the brief has a transformation to show, not only an argument to make. The scrubbed frame sequence, the shader ground, the scramble-decode and the flicker reveal are all exemplar-proven on this line. *Single-canvas monolith* (`engine-world`; Rogier, technique / single-source): one WebGL scene is the page, screens rotated on a circular path.
...
|---|---|---|---|---|
| hero | full-bleed cinematic still behind a left-aligned masthead (argument); a type intro playing over a full-bleed figure (gallery) | h1 per-char masked reveal \| scramble-decode; CTA row masked label roll-swap + token-wipe inversion; media clip-path masked reveal \| grayscale-to-colour curtain wipe \| inverse-scale parallax | 8 | the argument hero's CTA must answer the pointer (token-wipe inversion) — a pointer-dead hero is a defect; the entrance fires on load; on touch the entrance still plays and the CTA answers `:active` |
| proof | static logo wall | — | 4 | grayscale at rest → color on hover, one logo at a time; never a marquee, never autoplay — this is the designed rest |
| index | index header + row list | h2 masked line reveal; rows spotlight-dim; actions accent recolor + underline draw | 5 | hovered row lights an accent rule and surfaces metadata, siblings dim to 45%; plain tap target on touch |
| stat band | stat band | recoloring stat odometer | 5 | rolls on scroll-into-view (positional, re-fires); the markup carries the true final value with no JS; a quiet micro-climax |
| mechanism-climax | giant type as the image; one full-bleed figure (gallery variant) | caption scrubbed per-char recolor; media clip-path masked reveal \| curtain wipe riding inverse-scale | 9 | **the** one loud peak — giant type carries the product noun, or the first full-bleed figure lands; scroll-scrubbed so it re-fires on pass, never a fire-once flash |
| chapters | text/media split chapters (×3, media alternating) | h2 masked line reveal; prose scrubbed per-char recolor + key-term accent ignition; media clip-path masked reveal \| contained zoom + companion cue | 5 | prose brightens dim-to-bright as the block traverses (reversible, re-fires); marked key terms ignite to accent on first view; standalone headings carry near-zero hover |
| close | closing panel: one imperative + contact channel rows | ask masked line reveal; channels accent recolor + underline draw + masked label roll-swap | 6 | one imperative, decisive channel rows (accent recolor + underline wipe on hover/focus); no media slot, so the close cannot become a mood reel |
| footer | oversized wordmark (argument — deferred h1); bare scroll cue (gallery) | wordmark masked line reveal | 6 | the optional second quiet peak — the h1 lands last on a sticky reveal, legible with no JS; the portfolio close is just the bare cue |
...
**The hero beat.** For the cinematic and gallery registers, commit the medium immediately — the first impression forms here. A reading-first `standfirst-stack` instead opens on a clear typographic statement, honoring the brief's explicit exclusion of spectacle. `argument-scroll`: split curtain + recoloring odometer + per-char cascade over the cinematic still + exactly one CTA. `gallery-stack`: numeric preloader → scramble-decode type intro over the main gallery, first cells opening on masked inverse-scale reveals.
...
- *editorial chapters* — **continues quiet**: masked line and char reveals fire per section while the positional thread runs under them; the scrubbed per-char recolor brightens words as each block traverses. Never silence.
