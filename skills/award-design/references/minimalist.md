...
**Mid-page life** — the prose zone between hero and footer is carried by one positional décor channel riding under fire-once content reveals: canvas frame-sequence, scroll-scrubbed `clip-path:var()`, or a `mix-blend-mode:difference` seam label — positional means no fired state, so it re-fires every pass while the masked reveals and the odometer count-up persist; merely-good builds fire everything once on an IntersectionObserver and read inert on the second pass (Terminal Industries 7.68 + Gabriel Contassot 7.34, winner-verified mechanism). Hover on text is near-absent on prose and standalone headings tier-wide, and that absence is the register: the whole verified surface is `a:hover{text-decoration:line-through}` under `@media(any-hover:hover)` (Stefan Vitasović 7.25, winner-verified), `color` to the accent over `transition:color .3s` on nav/logo/phone links, `filter:brightness(1.05)` on one number, and one card-title `text-shadow` deepen (Terminal, winner-verified) — no per-char rise, no weight shift, no highlight on reading copy. Gabriel ships zero `:hover` rules and still takes SOTD — the life is scroll-driven directional masks plus the blend seam (winner-verified). The recoloring stat odometer — digits sliding inside `overflow:hidden`, gray → accent → ink — is the sanctioned mid-page climax (Terminal, winner-verified).

**Scroll texture** — What carries the eye down the page between interactions: a masked figure reveal riding inverse scroll-scale (Gabriel Contassot, winner-verified), canvas frame-sequence photography scrubbed by the scrollbar, or the scroll-scrubbed per-char recolor sustained top-to-bottom — Terminal runs it the full page height, so the text substrate itself is the carry (Terminal Industries, winner-verified). The design_plan names one: the committed carry, never a hoped-for side effect of the section reveals. Wheel smoothing is near-universal at the tier — Lenis on 3 of 4 winners (Terminal live-verified `html.lenis`; Treize `lenis@1.1.13`; Gabriel `1.0.42` surfaced in the shipped bundle on adversarial re-read — only Stefan runs without it); ship Lenis by default.
