...
What separates 7.5 from 8+ once the universe, layout, and motion are right: decisions measured in optical units, not tokens. Model defaults are geometrically correct and optically wrong — none of this gets applied unless it is applied deliberately. Loads at the chunk that commits the surface; build with it, never retrofit it.
