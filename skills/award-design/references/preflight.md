...
- [ ] **Medium arbitration anchored** — when the primary verb is a physical action on a world-object, OR the brief commits an immersive world (a scroll/scrub/ambient journey — immersive-cinematic / experimental): the arbitration is quoted verbatim in the pre-build R1 verdict (a Phase-5 paste is retroactive fiction) AND Assessor B's declared-vs-code check confirms the shipped code carries the arbitrated medium's fingerprint **as a presence gate** (element exists) AND — the liveness clear — Assessor A's driven medium box below shows the medium renders and answers; this box is never self-ticked alone, and B's code-read alone never clears it (`signature-invention.md`, `audit-rubric.md`)
...
| [ ] **Text-effect commit** | ≥ 1 named text effect from the design_plan's ingredient-set, present and driven (`text-effects.md` / the library's text components) | Global — zero text effects is a skipped Phase 4 commit, never a restraint choice |
| [ ] **Spectacle commit** | ≤ 1 committed spectacle moment, present and driven (scrub, gate, shader surface, filmstrip, or authored equivalent); zero only with the design_plan's archetype-canon citation | Global — an undeclared zero is the tame page; two competing climaxes is the fairground |
| [ ] **Motion continuity** | zero fully-static sections in the default render, footer included — every section runs the signature vocabulary at ≥ micro amplitude AND carries ≥1 driven DOM-layer response (an ambient scene never discharges the floor alone), `n/n` cited; the footer closes on the live signature | Global — a page that goes still after the hero fails however strong the hero is; a live canvas over dead DOM is the same failure in costume |
| [ ] **Legibility floor** | no text below ~11px effective at 1440; every strip, label, and CTA sitting over photography passes AA at its WORST rendered point (scrim, backing, or reposition — judged over the actual pixels, never the token background); index numerals and HUD readouts read at a glance | Global — "a bit too small / unreadable on its ground" is the recurring UAT tell; keep the register, raise the floor |
| [ ] **Static-frame integrity** | with motion off (`prefers-reduced-motion` + JS kill), every section frame reads as a deliberately composed layout; the climax resolves to a poster frame, and holds ≥ 30 fps on a mid-range device with motion on | Global — the reduced-motion state is the frame a juror tests, not a fallback |
| [ ] **Media-response coverage** | every figure/media surface carries its design_plan-named response, `n/n` cited | Global — an uncovered media element is an unfilled roster row `(detector: SUBSTRATE-DEAD)` |
| [ ] **Form-slot integrity** | every direct child of a `[data-ad-form]` root carries a contracted `data-slot` `(scanner: FORM-SLOT)` | Global — a slotless child is freeform layout smuggled inside a form-owned section |
...
| [ ] **Pacing curve** | the design_plan's per-section intensity holds on the page: at most one climax (the signature; zero only by declared archetype-canon) and ≥ 1 rest; every section within ±1 of the others is a flat curve | Global — a page of equally loud sections is a template |
...
| [ ] **Interaction-palette distinctness** | ≥3 element classes (CTA, link, figure/card, nav) carry distinct state mechanics, each named in the design_plan from the archetype's *Effect palette*, bound by one declared grammar (motion register + accent role — registers never mix; token count inside the register is free) | Global — one mechanic recycled across classes is the fail; distinct mechanics, one grammar (`interaction-signatures.md`) |
...
| [ ] **Class-role uniformity** | within each element class, every instance carries the identical treatment page-wide (all figures one response, all index rows one response, all text links one move) — role decides the treatment, placement never mutates it; a role's treatment lives in its library instance ONLY — build CSS never selects a component role class (`.ad-*`, `.is-primary`) `(scanner: ROLE-RESTYLE)` | Global — two figures answering differently without a committed role difference is incoherence, not variety; the hand-restyled "primary" beside a library primary is how one label grows two hover verbs |
...
- [ ] **Asset fidelity — measured, one row per signature asset**: `source px · shipped px · rendered device px at the worst moment` (deepest scrub zoom, largest cover-fit, hover scale). Scrubbed/zoomed sequence frames hold `rendered ≤ shipped` (ratio ≤ 1.0); static full-bleed photography holds ≥ 1.0× CSS pixels AND zero visible softness at rendered scale (film-treated stills may carry further only under a committed grain/grade/motion treatment, judged in-browser); anything under 1.0× of CSS pixels is disqualified; a scrubbed sequence is distinct REAL frames (footage/render/drawn — synthetic in-betweens baked from stills have zero winner precedent; stills animate live at full res instead); shipping a downscaled emission of a larger source you hold is the byte-trade the phased budget bans `(ref: imagery.md — Native resolution or nothing)`
...
- [ ] **Immersive medium is live, driven** — on an immersive-cinematic / experimental build, the committed primary medium is driven and shown to *render and answer*, not merely to exist: scroll the world and watch it move as a rendered scene / scrubbed sequence (the scroll drives `currentTime` or the scene), and where the medium is pointer-coupled, move the pointer and watch it respond; a medium that shows no render and no response under the drive is inert — the static-image-procession-with-décor failure — recorded with provenance (the tool-call / trace reference), never inferred from the code. Assessor B's fingerprint proves the element is present; THIS box proves it is alive, and only the driven eye can — canvas and video pixels are invisible to computed style, so no `run()` finding owns it. A build that commits the medium in the DESIGN.md but ships static display fails here `(refs: signature-invention.md, immersive-cinematic.md)`
- [ ] **Looping video loops seamlessly** — any `<video loop>` watched through ≥2 full cycles: the last frame resolves into the first with no jump-cut at the seam; a hitch is a fidelity fail, named in the verdict — cut on matched motion, crossfade, boomerang (`/video-loop`), or play once and hold, never loop a stutter. Self-ticking "loops cleanly" over a stuttering clip is the failure; the owner is the driven eye `(ref: immersive-cinematic.md)`
...
- [ ] **Moving window is compositor-clean** — any pointer/scroll-tracked window (a loupe, a torch/spotlight, a follow-reveal, a tracked comparison wipe) is driven with a fast programmatic sweep under a performance trace: across the sweep the tracked layer shows zero per-frame Paint/Raster/Recalc-Style events (Composite Layers only), no long frames, no stale-frame pop in the revealed content; every appear/park/clamp is a continuous eased trajectory, never a discontinuous jump. A paint-per-frame during the sweep IS the pop — a fail, not a gap; the trace reference is the provenance. Screenshots are a declared proxy only — a single composited frame cannot prove the absence of a pop (`motion-palette.md`, moving windows) `(scanner: MOVING-BG-POS, BG-ATTACH-FIXED, TRACKED-CLIP, TRACKED-ORIGIN)`
- [ ] **Placement pass — one placement row per standard capture** — the standard capture set (hero at rest, each section centered, the footer — 1440 and 375) judged BESIDE the archetype exemplar's corresponding frame, hunting the six named defects of `audit-rubric.md`'s placement pass (crowding · baseline misalignment · stretched meta-line · orphan label · text-over-image contrast · dead-zone/sprawl), each verdict measurable in the frame; every hit fixed or filed. One ledger row per capture (capture ref · exemplar ref · six verdicts · fix/filed) — a capture with no row is an unticked box. Driving proves response; only this pass proves composition `(ref: audit-rubric.md placement pass)`
...
- [ ] **Live substrate — every element responds, perceptibly** — driven (not code-read) at every section, not just the hero: each interactive element (the wordmark, every link, image/figure, card, nav, control, the accent word) carries a state you can *feel* in one coherent vocabulary. A ~3% image scale, a barely-there tint, or a fire-once effect that leaves a static frame is homeopathic — it reads as dead, not restrained; the response must register to a real pointer. The wordmark and the accent word are not exempt. A hover-revealed secondary stays reachable under touch emulation, never trapped behind a fine-pointer hover. **Under touch emulation the press-class elements answer the TAP** — every primary CTA/button/struck object flashes on `:active`/pointerdown, read at peak-hold (a substrate that only answers hover reads dead under `hover: none`); pointer-only classes (cursor, proximity, pointer-parallax) resting dormant is the winner answer, never filed as a gap. **On a press/strike-verb build, the input channel is driven too**: elements sampled by the reviewer from the detector's substrate census — never the builder's shortlist — answer a tap/click with the quiet verb instance, read at peak-hold `(ref: interaction-signatures.md)` `(detector: SUBSTRATE-DEAD)` `(detector: CONTACT-GLOBAL-SQUASH)`
...
- [ ] **Nav under momentum** — one instrumented full-page inertial scroll-down: zero nav un-hide events (sample the hidden state per frame, count flips — eyeballing misses an 80ms flash); the accumulator machine of `navigation-patterns.md` (cumulative travel + the dy==0 hold) is the pass, a raw-delta direction test is the bug
...
No browser tooling on the harness → this section's boxes convert to **declared gaps** in the verdict (Tooling gaps field), each with the code-level fallback noted — and the field carries the **verbatim failed probe** (the ToolSearch or `command -v` output), never a bare claim. Gaps cap the status: with this section dark the ceiling is **READY-UNVERIFIED** (the label travels verbatim into the ship message), and a design_plan that committed an interactive signature — OR any immersive-cinematic / experimental build, wired to the archetype and NOT to the self-classified verb — caps at **NOT DONE — unverified render**. The archetype wiring is the point: a build that labels its own verb "scroll, non-interactive" to duck the cap does not escape it — the immersive medium is unverified until driven live, whatever the build calls its verb (the exact escape that shipped a static-display immersive page at READY-UNVERIFIED). Falsely ticking a browser box is worse than declaring the gap; declaring a gap the harness could have closed is the same fail.
