...
- [ ] **Reversible scroll motion** — on-scroll reveals replay: scroll a section past, then scroll back up and down again — the animation re-runs, it does not sit spent and static (the fire-once IntersectionObserver tell). Native `animation-timeline` scrubs are reversible for free; an IO fallback re-arms on exit `(refs: motion-palette.md)`
