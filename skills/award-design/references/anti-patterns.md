...
- **Rotation memory** — do not reuse the previous build's palette family, type pairing, hero layout, **or reading kit** (the `h1-entrance` + `prose-substrate` slots). If the last premium-consumer build ran sand+brass, this one runs a different family; if the last minimalist build ran kinetic-reveal + text-emphasis-fill, this one substitutes a same-role id from `slot_roles`. The reading kit is a real axis, not DNA: the kinetic-reveal + text-emphasis-fill + semantic-accent + clip-reveal set shipped across five different-brand builds because nothing named it. Two sources feed it: the previous build's contract — its FORM+SEED block, which survives in the artifact's opening comment and names the spine and the roll key — and this session's builds. State the rotation out loud at the concept steps — rotating in your head is how the house look creeps back.
...
- **Default accordion FAQ sections** — the lazy reach; prefer side-by-side lists, searchable help, or inline progressive disclosure. An accordion earns its place only as a deliberate, styled choice (the library's faq-accordion form is one).
