...
- **Rotation memory** — do not reuse the previous build's palette family, type pairing, hero layout, **or reading kit** (the `h1-entrance` + `prose-substrate` slots — the stamp's `text:` field). If the last premium-consumer build ran sand+brass, this one runs a different family; if the last minimalist build ran kinetic-reveal + text-emphasis-fill, this one substitutes a same-role id from `slot_roles`. The reading kit is a real axis, not DNA: the kinetic-reveal + text-emphasis-fill + semantic-accent + clip-reveal set shipped across five different-brand builds because nothing named it. Two sources feed it: the previous build's stamp (first line of the main stylesheet, written at Phase 4 — format defined there, once) and this session's builds. State the rotation out loud in the Phase 1 artifact — rotating in your head is how the house look creeps back.
...
- **A landing that describes and never asks you in** — rules, specs, price, address, and no answer to why this exists, for whom, who is there, what makes it exceptional, why come now. The five answers are content with named sections behind them, never a tone. Description informs; desire moves (the desire arc, Phase 1).
