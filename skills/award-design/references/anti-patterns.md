...
9. **Never wrap an H1 across 4–6 lines.** Use ultra-wide containers (`max-w-5xl`, `max-w-6xl`, or `w-full`) and scale type with `clamp()` so the headline lands in ≤2 lines — a 3rd takes a written override, and the detector FAILs past 2 line boxes. The narrow-container 6-line wall is the canonical AI-headline failure. See `premium-patterns.md` for the 2-Line Iron Rule.
...
- **Rotation memory** — do not reuse the previous build's palette family, type pairing, hero layout, **or reading kit** (the H1 entrance and the body-text substrate). If the last premium-consumer build ran sand+brass, this one runs a different family; if the last minimalist build ran a masked line reveal over a scrubbed per-char recolor, this one substitutes a different mechanic in each of those two roles. The reading kit is a real axis, not DNA: the same masked-reveal + scrubbed-recolor + key-term-accent + clip-path-reveal set shipped across five different-brand builds because nothing named it. Two sources feed it: the previous build's contract — its FORM+SEED block, which survives in the artifact's opening comment and names the spine and the roll key — and this session's builds. State the rotation out loud at the concept steps — rotating in your head is how the house look creeps back.
...
- **The muted world** — a subject whose real objects carry a canonical colour system (calibrated kilo plates, race liveries, signal flags, process inks) rendered monochrome-plus-one-accent for taste. The world's own colour code is a palette the brief already handed you — adopt it with declared roles (each hue keeps its real-world meaning); muting it reads as a brochure *about* the thing, never the thing. Gaiety is not a register violation — even brutalist winners field full saturated accent fills.
- **The uninhabited world** — every frame a still life: objects, rooms, product, landscape, and nothing lives there. The world's presence — human, animal, machine, vegetal, or the moving element itself (wind through the dust) — appears *in motion* when the world genuinely has one: a Mars robotics page shows its robots working; a pure desert still carries its wind. A quiet reading register may earn stillness — declared, never defaulted.
...
- A `border-bottom` hung under the nav bar — at rest, on scroll, or lighting up on hover — as the bar's separation affordance. The bar's solid surface is the page ground or the dominant primary, and that opaque ground is the separation (`navigation-patterns.md` Surface); a different-colored line under the bar reads as template chrome, and one that appears on hover reads as a glitch.
...
- **The brochure hero** — hero copy that names the category ("A powerlifting club in a freight depot") instead of making the promise ("A new era of enduro starts here"). The description says what it is; the promise says where it takes you. Lead with the promise; let the description support.
- **A landing that describes and never asks you in** — rules, specs, price, address, and no answer to why this exists, for whom, who is there, what makes it exceptional, why come now. The five answers are content with named sections behind them, never a tone. Description informs; desire moves (the desire arc, committed in the contract's STORY block).
- Undecodable data marks — dots, bars, or glyphs a stranger can't name. Self-evident marks need nothing; ambiguous ones take a one-line legend. The loop's screenshot is the test: can someone who didn't build it say what each mark is? The same test binds a drawn in-world object: a diegetic diagram (a barbell, a dial, a gauge) reads as its referent in one glance, drawn whole with its real anatomy — a barbell carries a sleeve at *both* ends; cropped or abstracted below recognition, the object stops carrying its metaphor.
...
- Any special-state native cursor as an affordance — `zoom-in`/`zoom-out`, `wait`/`progress`, `help`, `crosshair` — the OS glyphs deface the design the same way `not-allowed` does. Design the affordance: a custom cursor where the register earns one, or a real control (a labeled zoom button, a skeleton state) instead of a cursor promise. System `default`/`pointer`/`text` stay legitimate — winners keep them.
- The native focus ring left on mouse click — a `:focus` outline (or the browser-default ring) flashing on every click or tap reads as unstyled chrome. Focus styling lives on `:focus-visible` (keyboard-only), custom-drawn; inputs take a designed focus state in the committed accent, never the OS blue.
- A `#top`/`#home` fragment left in the URL by the wordmark or a back-to-top control — chrome navigation scrolls by script (the wordmark: home from an inner page, smooth scroll-to-top on the homepage, reload when already at top — `navigation-patterns.md`); a hash appearing in the address bar on a chrome click is the unfinished-anchor tell.
...
- **The pale hover nobody chose** — a control's hover state painted in a washed tint (a high-lightness wash of the accent, or a literal that lives in no token sheet) while the page's links answer in the committed accent. The state color of a button is a full-strength committed token — the interactive accent the DESIGN.md names — never its faded sibling; two half-strength decisions on one page read as no decision (`interaction-signatures.md`).
...
- **Default accordion FAQ sections** — the lazy reach; prefer side-by-side lists, searchable help, or inline progressive disclosure. An accordion earns its place only as a deliberate, styled choice (a styled accordion is one).
...
- **"MENU" or "CLOSE" text beside the nav toggle icon** — the toggle is icon-only (hamburger ↔ cross, named by `aria-label`), morphing in place at one point so the pointer never travels between open and close (`navigation-patterns.md`). A `✕` that appears across the viewport from the burger is the teleporting-close tell.
