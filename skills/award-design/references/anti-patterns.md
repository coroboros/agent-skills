...
- Fire-once scroll reveals as the motion model — an IntersectionObserver that `unobserve`s after the first intersection plays once and never again, so a second pass down the page shows dead, static content that already spent its one animation. The default is reversible scroll-linked motion (native `animation-timeline: view()/scroll()`, `motion-palette.md`); where an IO path exists it is the *re-arming* Safari/Firefox fallback (`removeAttribute` on exit, replays on re-entry), not the mechanism. A `once` reveal is reserved for the rare single-shot element (a hero wordmark).
