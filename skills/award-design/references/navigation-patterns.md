...
The archetype register sets the flavor over the persistent-bar default (`award-imperatives.md` per-archetype table): Corporate Luxury and Minimalist run a quiet bar, scroll-aware only where the reading earns it; Editorial a quiet or scroll-aware header, or a dual-menu editorial nav; Bold/Maximal an overlay staged as an event; Immersive a minimal HUD-like bar or overlay; Experimental makes the bespoke navigation metaphor the signature itself, with a conventional escape hatch. Whichever register, the axes above still hold — a bar that reappears transparent over content is the tell regardless of flavor, and so is one that flickers under scroll jitter.
