...
- **Motion 1-3** → opacity/color transitions as the base (`transition: opacity 0.4s`); scroll-triggered reserved for the signature and one felt text effect (emphasis on already-legible copy, `text-effects.md`) at low amplitude. Ambient follows the selected archetype and committed choreography (interaction-signatures.md); minimalist may omit idle while retaining driven motion. Guard any chosen loop for reduced motion. **Motion 7-10** → GSAP ScrollTrigger pin/scrub on hero, View Transitions on navigation, perpetual micro-interactions on signature elements (memoized per `premium-patterns.md` performance locks).
