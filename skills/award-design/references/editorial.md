...
**Figures / cards**
- **Contained zoom.** Siena `.previousnext-item:hover .full-img-w { transform:scale(1.1) }` inside `overflow:hidden`. Baseline, present everywhere. (Siena SOTD Mar 2025)
...
- **Transparent + gradient scrim + ink-inversion by section (dark cinematic).** Siena `.nav-w`: `position:fixed`, `pointer-events:none`, no bar; legibility from a `linear-gradient(#000,#fff0)` top scrim; a section class flips ink (`.on-dark .logo { color:#000 }`). (Siena SOTD Mar 2025)
...
- **Type-as-image masthead.** One display word owning the frame; Siena's vintage-poster masthead over cinematic stills is the dark-register expression. (Siena SOTD Mar 2025)
