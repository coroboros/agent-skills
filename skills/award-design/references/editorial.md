...
**URL.** `siena.film`
**Award.** Awwwards Site of the Day, 2025-03-18 + Developer Award — 7.9 overall (Design 7.99 / Usability 7.61 / Creativity 8.13 / Content 8). Site of the Month March 2025 nominee only: the vote page carries it, the site's Awwwards page carries no SOTM badge.
**Studio.** G-NS Studio.
...
**Buttons / CTA**
- **Full-fill + ink inversion (primary/secondary).** Outline or light button fills a *solid* brand value, label flips to the opposite ink — never a pale tint. Siena's all-work CTA floods to solid with its nested arrow inverting the opposite way (mechanic verified, selector and `.5s` timing internal-source); Anthropic play button bg↔text at `.2s`; Truekind `.btn.outline:hover { background:#3b3b3b; color:#fff }` as the generic cross-archetype form. Default for the main CTA. Library id: `fill-invert-cta`. (Siena SOTD 2025-03-18; Anthropic control; Truekind cross-archetype)
- **Two-layer kinetic label roll (signature hero CTA).** Siena `[data-btn=explore]`: SVG shape fills transparent→white, stroke flips to black, and the label *rolls* — visible copy slides `translate(100%)` out while a duplicate below rolls `translateY(-150%)` in, both recolouring to the ink, staggered `.1s`; `--duration:.8s` on `--easeOutQuint`. The loud move — fill *and* kinetic label. Library id: `masked-label-swap`. (Siena SOTD 2025-03-18; mechanic named by the Awwwards entry, transforms and durations internal-source, single-source)
...
**Links**
- **Underline drawn from origin-left.** `.link:hover:before { transform:scaleX(1); transform-origin:0 50% }`, retracting rightward when already active. A drawn line, never a snap. Library id: `accent-link`. (Truekind — a light skincare SOTD; the mechanic is generic, the citation carries no editorial-dark weight)
...
- **Randomized per-letter transformation on hover.** Bisous menu links replay the stochastic letter settle rather than drawing a line — the dark-cinematic variant, rhyming with the page's type texture. Library id: `flicker-reveal` (hover-replay mode). (Bisous, Codrops 2026-06-29; easings and durations described qualitatively, never published)

**Figures / cards**
- **Contained zoom.** Siena `.previousnext-item:hover .full-img-w { transform:scale(1.1) }` inside `overflow:hidden`. Baseline, present everywhere — felt, never a 1–3% twitch. Library id: `figure-hover`. (Siena SOTD 2025-03-18; selector internal-source)
- **Focus / defocus siblings.** The hovered figure sharpens while neighbours blur and dim, so attention is *directed* — Dondre stories page, WebGL displacement. Pick for a gallery where one item should win the eye. Library id: `focus-defocus`. (Dondre Green, Codrops case study)
- **Index footage preview.** For a body of film work the canonical row reveal is the footage itself: hovering a project title surfaces its muted video in one cursor-attached floating layer, a marquee title gliding over it. Stronger than the text-only spotlight-dim on this archetype's content. Library id: `hover-preview-video`. (Bloom "Hover listing archives", SOTD 2025-05-07 + Codrops 2025-07-08)

**Nav** — three verified treatments, none frosted, none with a contrasting `border-bottom`:
- **Transparent + gradient scrim + ink-inversion by section (dark cinematic).** Siena `.nav-w`: `position:fixed`, `pointer-events:none`, no bar; legibility from a `linear-gradient(#000,#fff0)` top scrim; a section class flips ink (`.on-dark .logo { color:#000 }`). Library ids: `nav-context-ink` + `show-on-scroll-up-nav`. (Siena SOTD 2025-03-18; selectors internal-source)
...
- **Index row** — on a body of film work the reveal is the footage (`hover-preview-video`, commonly cursor-tracking); text-only lists fall back to the row lighting an accent rule with siblings dimmed to ~45% (`index-row-hover`), `~.3s` on the signature ease. Touch navigates on tap or plays inline, with a brief press flash and no sibling dim. `:focus-visible` lights the row — or shows a poster frame — identically.
...
- **Nav** — links recolor and underline, or roll per character; the bar is a `pointer-events:none` gradient scrim over the hero that gains ground on scroll and inverts ink by the section behind. Tap opens the dual-menu overlay. `show-on-scroll-up-nav` reveals the bar, focus rings sit on the links, and the scrim never traps focus.
...
**Section chain** — the section roles with their intensity map and the state each owes. The intensity numbers are ordinal authoring targets, not measured DOM reads; the shape they encode (hero dominant, reel near-hero, detail reduced-but-nonzero, close a single re-expression, no silence) is what is verified. Pick forms by role; never hand-write hero or section layout CSS.

| role | form | pairs | intensity | state it owes |
|---|---|---|---|---|
| threshold | `gated-splash`; loader-scene (continuous-surface) | overlay `grain-grade`; enter `fill-invert-cta` \| `masked-label-swap` | 7 | the ENTER control MUST answer hover/focus — a dead ENTER hover is the first impression lost; doubles as the sound gate; authored hidden so no-JS shows the page |
| hero | `hero-masthead(media:back)`; `type-as-image` (poster masthead) | h1 `kinetic-reveal` \| doubled `masked-label-swap` \| `flicker-reveal`; media `scrub-film` \| `clip-reveal`; grade `grain-grade`; data-strip mono stamp; cursor `cursor-verb-label` | 9 | `pointer-events:none` gradient-scrim nav over the still, never a bar; the entrance IS the signature; the hero hands over the verb reused below — a pointer-dead hero is a defect |
| reel / proof | `pinned-filmstrip` (desktop hold-and-drag); `fullscreen-vertical-reel`; `drag-scrub-video` field | caption `text-emphasis-fill`; media `image-curtain`; rows `hover-preview-video`; cursor `cursor-verb-label`; mobile `swipe-snap-gallery` | 8 | the operable-drag verb sustains at near-hero amplitude — grab-drag on desktop, native scroll-snap on touch; rows reveal their muted footage on hover; titles roll on the same doubled label; jury pull-quotes borrowed, never claimed |
| feature / detail | `editorial-split` (media left/right); `stacked-card-scroll` | h2 `char-assemble`; prose `text-emphasis-fill`+`semantic-accent`; media `image-curtain`; spec `counter-odometer` | 6 | REST but not silent — the grade and one text signature stay alive; figures answer hover with contained zoom; no hover on in-flow headings; measure held at 60–75ch; reversible image parallax legal, prose never scrubs |
| close | `close-panel`; contact modal (Bloom) | ask `kinetic-reveal`; channels `accent-link`+`masked-label-swap`; trust mono line | 6 | re-spends the thread exactly once — one imperative (18ch cap), decisive channel rows answering hover and tap, a quiet trust line a full rest below; no media slot, so the close cannot become a mood reel |
| footer | `oversized-wordmark`; bare-cue / costumed-credits (continuous-surface) | wordmark `kinetic-reveal` | 6 | the grade and the thread reach the footer; an oversized signed wordmark, or a designed thin credits refusal when the body already spent the spectacle; the palette may flip once here |

**Footer** — *wordmark-contact-close* (Siena, winner-verified copy): "LET'S TALK" / "EMAIL US" / lee@siena.film / "©2024. SIENA FILM FOUNDATION.". *Tabular-index* (Anthropic, winner-verified columns): ivory wrap, same-family hairline `border-top` (`--swatch--ivory-medium`), serif links (technique). *Social-grid-functional* (Truekind, winner-verified copy): "Connect With Us" / "on instagram" grid, "Website By:" credit. *Costumed credits* (continuous-surface): a thin credits close is a designed refusal, not an omission, once the body has already spent the spectacle. Only the oversized wordmark has a library form — the tabular-index and contact-first footers are still `MISSING:` entries in the recipe set; author them at form quality rather than improvising chrome.
...
- *per-work detail* — REST, NEVER SILENT: char-assemble heading, treated stills, spec counters; the grade and one text signature stay alive while the amplitude drops (Siena).
...
- `continuous-work-transition` — The curtain-LESS work-to-work seam: go(fn) names the outgoing figure, runs the swap inside a View Transition, names the incoming figure — the pair morphs…
...
- `cursor-verb-label` — The visual teacher for the operable verb: over a field declaring data-ad-cursor-verb the cursor morphs to its verb label ('DRAG', 'HOLD', 'VIEW', a play…
...
- `fullscreen-vertical-reel` — The full-screen vertical slider that IS the whole home: one full-bleed treated work per panel, infinite in BOTH directions (modular wrap — no start, no end)…
...
- `stacked-card-scroll` — The pinned deck that peels: each card pins on a NATIVE sticky (no scroll hijack), the next rises over it, and the covered card scales/lifts/dims in proportion…
