...
3. **Smooth scroll + scroll-as-narrative, reversible by default.** Content unfolds with pacing, not uniform per-section fades. The default motion model is **scroll-linked and reversible** — animation tied to scroll position that unrolls on the way down and re-rolls on the way up, replayable every pass (`motion-palette.md`). Native CSS scroll-driven animation (`animation-timeline: view()/scroll()`) is the primitive; a fire-once IntersectionObserver reveal that plays once and never again is the tell it replaces (the IO path, where it exists, is the re-arming Safari/Firefox fallback, not the mechanism). `Lenis` (or an equivalent that preserves native `position: sticky`) smooths the scroll where the register wants it. The model is imposed; the *amount* of motion stays governed by the archetype's restraint. One climax, at least one rest — never every element fading in on scroll.
