...
Loads at truth sourcing (step 7) for the mechanics the signature and the motion model need, and again at the build step.
...
**Track with transforms; reveal pre-promoted content — never track clip, origin, or background position over a scaled or filtered raster.** A window driven per input frame (a loupe, a torch or spotlight, a comparison wipe that follows the pointer, a follow-reveal) is two compositor-only layers: the window moved by `transform: translate3d`, its content counter-moved by `translate3d` in the SAME rAF write; the clip is STATIC (`overflow: hidden` / `border-radius` / a fixed `clip-path`); `will-change: transform` on both so each owns its layer; content pre-decoded (`img.decode()`) before first show. Every appear, park, and clamp of the window is a **continuous eased trajectory** — a clamp that teleports its target makes the lerp lunge, which reads as the same pop. The failure signature: a per-frame repaint (a moving `clip-path … at`, an animated `transform-origin`, a JS-written `background-position`) desyncing from the compositor-cheap geometry — smooth input, popping output; the compositor presents last-good raster under fresh geometry, and bright content flashes stale positions (a shipped loupe did exactly this: `clip-path: circle(… at var())` + `transform-origin: var()` chasing the pointer over a 2.1×-scaled raster, no layer promotion).

**Scope — per-frame-TRACKED windows over transformed content only.** Fire-once reveals (an iris growing its radius, a curtain/inset wipe, a masked-text reveal), comparison sliders clipping a 1:1 unscaled raster, and static or scroll-scrubbed masks (seam grades, parallax layers) correctly USE animated `clip-path`/`mask` — never translate-rig those. Corollary: never `background-attachment: fixed` (the mobile banding sibling) — translate a layer instead. Verified compositor-clean at the verify phase with a performance trace over a fast programmatic sweep — Composite-only frames, zero per-frame Paint/Raster (`preflight.md` §8); screenshots cannot prove the absence of a pop.
