...
For Immersive, Experimental, Bold, and Spatial-Organic builds, the universe carries enough motion and dimensional detail to specify the signature in full. When that signature is a self-contained WebGL/R3F scene — the one delegation — this file is the subagent's sole brief:
...
- **3D / shader build notes** — when the signature is WebGL, point the builder to `references/ingredients/web3d-for-sites.md` (or `references/ingredients/ogl-shaders.md`); the scene expresses these tokens, never generic defaults.
...
The mirror authored or merged into `globals.css` from these YAML tokens (the Tailwind exporter emits canonical JSON, not this CSS or extension tokens):
