---
name: frontend-dev
description: Build and improve everyday pages, dashboards, forms and components. Use for add a page, build a dashboard, design a form or make existing UI look better. Preserve DESIGN.md and explicit chunk scope; validate the affected surface and its applicable states. Execute award-design for award-level marketing sites and design-system for single-token edits when installed.
...
1. **A ladder chunk** — a pasted build chunk (Read first / Implement / Verify / Out of scope / Report), or a requested row of `design-plan.md`. When `/award-design` is installed, load and execute its chunk mode; naming the skill alone is not completion. Otherwise implement the selected chunk and its Verify as written, then report in its ladder row (or the response when no file exists). Missing scripts or browser capabilities remain declared gaps, never passes. Stop at the selected chunk boundary.
...
- The brief names the ceiling — "award-winning", "premium", "signature", a new visual identity, an uplift or redesign — on a landing, portfolio, product or marketing site → load and execute `/award-design` when installed. The task owner continues a full build through its phases; a direction-only request ends at direction. If the skill is absent, report the missing award workflow and deliver only independently useful authorized work with the unmet award requirement explicit. A dashboard or internal tool stays here at any ambition.
