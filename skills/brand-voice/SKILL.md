...
When `-s` is passed alongside `extract`, the skill also writes a copy to `~/.claude/output/brand-voice/{project}/voice.md` for pipeline-history consumers, where `{project}` is the repo basename (git toplevel, else cwd) per `.claude/rules/repo-conventions.md` § Output paths (path shown tilde-form). The canonical file at `./BRAND-VOICE.md` remains the single source of truth.
...
- **Output paths follow the repo contract.** Default canonical at `./BRAND-VOICE.md`. Pipeline copies under `~/.claude/output/brand-voice/{project}/voice.md` only when `-s` is passed.
