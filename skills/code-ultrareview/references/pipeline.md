...
Knip also needs a manifest to run. It executes only when a `package.json` at the repository root or in one workspace covers the changed JavaScript/TypeScript files, from that directory. With no covering manifest the battery records Knip as not applicable — `applicable: false` in `tool-preflight.json`, `tools-skipped.json`, and `scope.json["tools_skipped"]` — and continues; Phase 3 and synthesis accept that entry, and the report lists it under Tools skipped. Partial coverage, or several packages with no root manifest, exits 2 like an inconsistent declaration.
...
- Exit 0 with Knip recorded as not applicable when no `package.json` covers the changed JS/TS files.
