...
Python and native analyzers use installed `PATH` commands. Universal performance Semgrep rules live under `perf-rules/`. Markdownlint uses the bundled neutral base unless the project overrides it. `jscpd` defaults to 15 lines and 100 tokens; `JSCPD_MIN_LINES` and `JSCPD_MIN_TOKENS` may make the threshold stricter.
...
- Exit 4 for analyzer error, timeout, empty or malformed analyzer output, publication failure, or incomplete requested coverage. Preserve evidence paths and print exact repair plus rerun guidance.
