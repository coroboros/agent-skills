...
| `--mutation-test` | Run Stryker (JS/TS), Pitest (JVM), or mutmut (Python). Maven and Gradle run from `PATH` in offline mode. Python projects configured through `pyproject.toml` require Python 3.11+ for standard-library TOML validation. JS/TS execution targets changed files; project-configured Python/JVM execution is filtered to changed-file findings. Surviving mutants route to the Tests axis as 🟠 Medium |
