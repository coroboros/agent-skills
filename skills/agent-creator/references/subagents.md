...
model: sonnet # Optional - model alias, full ID, or 'inherit'
---
...
**Model resolution**: `CLAUDE_CODE_SUBAGENT_MODEL` env var > per-invocation model > frontmatter model > main conversation model.
...
- **Use for**: Planning, code generation, validation, most agent work
...
| `subagent_type` | string | No | Type of specialized agent to use |
...
**Resuming agents:** Use `SendMessage` with the agent's ID to continue where it left off with full context preserved.
...
# Results arrive automatically when each completes
```
...
```
SendMessage:
- to: "abc123"  # Previous agent_id
- content: "Please continue with the remaining files..."
```
