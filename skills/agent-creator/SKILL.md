...
These four facts are stable: production-observed configuration footguns. Cite file:line or a reproducible condition.
...
3. **Subagents cannot spawn other subagents.** Per § Execution Model, including `Agent` in a subagent's `tools` is rejected at runtime. Multi-agent fan-out must happen from the main thread (or a `--agent`-launched main agent), never from inside a subagent. Fix: flatten the workflow — main thread orchestrates the parallel subagents directly.
