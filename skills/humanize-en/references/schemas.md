...
The consumer requires a non-empty `voice.name` and validates the shapes of supported lists and mappings. Optional rules, empty lists and semantic-only fields are valid. Provenance `--explain-json` envelopes are rejected; use `--resolved-json`. Input failures exit `1` in prescan and `2` in validate, with a diagnostic instead of a hit report.
