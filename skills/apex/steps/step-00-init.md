...
save_mode: false # -s: Save outputs to ~/.claude/output/apex/{project}/
...
{task_id} will be generated by the setup script:
  - Script auto-detects next available number in ~/.claude/output/apex/{project}/
...
1. **Search for matching task:**
   ...
   ```bash
   ls ~/.claude/output/apex/{project}/ | grep "^{resume_task}"
   ```
...
- Creates `~/.claude/output/apex/{project}/{task_id}/` directory
...
**Capture script output into state variables** — the script prints `TASK_ID=NN-feature-name` and `OUTPUT_DIR=<absolute ~/.claude/output/apex/{project}/NN-feature-name>` on stdout. Parse those two lines and assign them to `{task_id}` and `{output_dir}` respectively (surface the path to the user in tilde form). Downstream steps depend on both.
