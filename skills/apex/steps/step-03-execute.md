...
Run the project's typecheck and lint commands (discover from `package.json` scripts or CLAUDE.md).
...
- **Don't cite the machinery.** No references to the plan, spec, brainstorm, postmortem, the APEX phases, or scratch-file paths (`~/.claude/output/...`) in anything that ships — the reader can't open them, so the pointer is a dead end.
