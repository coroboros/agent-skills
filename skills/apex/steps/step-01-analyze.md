...
1. **`{from_file}` is an explicit path — `Read` it verbatim.** The producer already printed its fully-expanded absolute path and the bridge command carries that literal path; there is nothing to reconstruct, infer, or glob. If the path does not exist, fail loud — ask the user to correct it or regenerate it via the producer (e.g. `/forge -s "<idea>"`). Then read the file with the Read tool
