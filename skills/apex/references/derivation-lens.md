...
| `SCOPE-ADD` | A change does not serve an accepted outcome, or adds a file, exported symbol, abstraction, dependency or config key beyond the plan's `Design budget` without a recorded deviation | Remove task-owned unnecessary changes. If useful but outside authorization or expressly excluded, surface the concrete decision for the user. |
