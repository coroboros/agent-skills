...
| `SCOPE-ADD` | A change does not serve an accepted outcome, or exceeds a `Design budget` count, estimated net lines aside, without a recorded deviation | Remove task-owned unnecessary changes. If useful but outside authorization or expressly excluded, surface the concrete decision for the user. |
