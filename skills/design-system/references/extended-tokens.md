...
5. **`globals.css` `@theme` mirrors YAML 1:1.** Author or merge the downstream CSS from the YAML mapping; preserve unrelated stylesheet content. `/design-system export tailwind` emits canonical JSON, not CSS or extension tokens. `/design-system audit-extensions` checks token names and references; inspect value parity separately.
