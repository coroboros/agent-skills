...
### Bias toward
- `font-feature-settings: "ss01"` on every sohne-var declaration
- Weight 300 for headlines — lightness as authority
- Negative letter-spacing at display sizes (`-1.4px` at 56px, scaling down)
- Blue-tinted multi-layer shadows (`rgba(50,50,93,0.25)` + `rgba(0,0,0,0.1)`)
- Deep navy (`#061b31`) over pure black for headings and nav
- Conservative radii (4-8px) — nothing pill-shaped, nothing squishy
- `"tnum"` on every financial number, never ss01 there

### Reject
- Weight 600+ on sohne-var — it reads like a different brand
- Pure black (`#000`) on any text, surface, or shadow
- Pill buttons, large radii, overly-rounded geometry
- Neutral gray shadows — every shadow carries blue brand color
- Warm accents (orange, yellow, warm reds) on interactive elements
- Positive letter-spacing at any display size
- Missing `"ss01"` on sohne-var — the stylistic set IS the identity

### Self-test question
"Does this look like generic fintech from 2018 or Stripe from 2026?" If it reads generic fintech, the letter-spacing is too loose, the shadows are neutral gray, or the weight is too heavy. Tighten tracking, tint shadows, lighten weight until the answer is unmistakably Stripe.
