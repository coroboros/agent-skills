...
Output path per `.claude/rules/repo-conventions.md` § Output paths. The three subcommands (`audit`, `diff`, `audit-extensions`) write sibling reports under the same `{project}/` directory.
