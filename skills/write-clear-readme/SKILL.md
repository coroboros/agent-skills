...
- When `$ARGUMENTS` is empty, default to `./README.md` if it exists; else propose the most recent README candidate from session context and confirm. Otherwise treat first token as subcommand and second as path.
