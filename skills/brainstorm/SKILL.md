...
| Flag | Behavior |
|------|----------|
| `-s` | Save the brief to `~/.claude/output/brainstorm/{project}/brainstorm.md` (global; `{project}` = repo basename) |
| `-S` | Force no-save (override any ambient save mode) |

Flags are removed from input; remainder becomes `{topic}`. `{project}` = kebab-cased basename of the git toplevel (else cwd) — see `.claude/rules/repo-conventions.md` § Output paths. Lowercase enables, uppercase disables — matches the repo-wide convention.
...
Produce the strategic brief. Output in conversation by default, or save to `~/.claude/output/brainstorm/{project}/brainstorm.md` when `-s` is set — derive `{project}` per `.claude/rules/repo-conventions.md` § Output paths, `mkdir -p` the directory, and show the path to the user in tilde form.
...
If the brainstorm leads to work that requires code, suggest the path forward based on scope. The bare `brainstorm.md` below resolves deterministically to `~/.claude/output/brainstorm/{project}/brainstorm.md` per the repo `-f` resolution rule — no path needed:

**Complex work** (multiple workstreams, needs planning):

```
/spec -s -f brainstorm.md "{topic}"
```
...
```
/apex -f brainstorm.md {description}
```
