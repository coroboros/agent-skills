...
Output is global — `$HOME/.claude/output/apex/<project>/` — so HOME is
...
def _run(*args, cwd, home):
    ...
    so git never walks above the temp dir, and an isolated HOME."""
    env = os.environ.copy()
...
class TestValidateState(unittest.TestCase):
    ...
    def test_invalid_step_num_rejected(self):
        ...
        result = _run("01-foo", "9", cwd=self.root, home=self.home)
        self.assertEqual(result.returncode, 1)
        self.assertIn("step_num must be 1-4", result.stderr)

    def test_missing_task_folder_exits_1(self):
        """No task dir under the global apex dir → exit 1, task-missing."""
        result = _run("01-nonexistent", "2", cwd=self.root, home=self.home)
        self.assertEqual(result.returncode, 1)
    ...
    def test_missing_context_file_exits_1(self):
        ...
        self._seed_task("01-foo", with_context=False)
        result = _run("01-foo", "2", cwd=self.root, home=self.home)
        self.assertEqual(result.returncode, 1)
    ...
    def test_full_chain_complete_exits_0(self):
        ...
        result = _run("01-foo", "3", cwd=self.root, home=self.home)
    ...
    def test_step_one_skips_prior_check(self):
        ...
        self._seed_task("01-foo")
        result = _run("01-foo", "1", cwd=self.root, home=self.home)
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertIn("RESULT: ok=true", result.stdout)
