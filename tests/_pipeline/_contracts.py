...
a producer saves output to `~/.claude/output/<skill>/<project>/`, a consumer
resolves it deterministically via `-f` (bare name → that path; explicit path
→ as-is). Schema drift between producer and consumer is where bugs hide —
...
CLUSTERS = {
    "workflow": {
        ...
        "producer": "brainstorm",
        "producer_output": "~/.claude/output/brainstorm/{project}/brainstorm.md",
        "consumer": "spec",
        "consumer_output": "~/.claude/output/spec/{project}/spec.md",
    ...
    "review": {
        ...
        "producer_output": "~/.claude/output/code-review/{project}/code-review.md",
        "consumer": "apex",
