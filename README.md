...
- **Review mode** — `award-design review <url|path>` is the always-on adversarial fresh-eyes: refute by default against the awwwards rubric (Nielsen usability heuristics + the concept veto) + the anti-slop catalog + the pre-flight checklist + the DESIGN.md, with cited fixes and an on-track / off-track verdict, never a silent pass. Runs internally twice per build — R1 refutes the universe before any file, R2 refutes the rendered site before ship — with **anti-anchoring order**: the reviewer judges the pixels first and reads the scanner report second, and closes with the **exemplar gap read**: three concrete gaps vs the archetype's canonical winner, each with a fix
