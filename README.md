...
Takes the lead on any frontend design, build, or redesign — an **art-director on a mandatory protocol** (phases 0–6, in order, every build) targeting Awwwards SOTD 7.5+, FWA, CSSDA. Forces a committed, anti-default visual **universe**, writes it as a **DESIGN.md** (the [Google open standard](https://github.com/google-labs-code/design.md)) when none exists, adapts to an existing one and alerts when it is thin, then **builds the frontend itself** under that direction — and refuses to ship until the **pre-flight gate** (a deterministic scanner + a box-by-box checklist) and a fresh-context adversarial review both pass. Frontend only — single-token tweaks route to `/design-system`, never backend. A **review mode** audits any site at any time.
...
- **The protocol is mandatory** — phases 0–6, in order, every build: read the room → conceive the universe → write the DESIGN.md → source the truth → commit-and-build → pre-flight → adversarial review. Each phase names the references to load *at that moment* (non-optional), ends with a stated artifact, and gates binarily — merging or skipping gates because the brief "felt complete" is called out as the dominant failure mode. Art-director judgment stays ambient on top; it no longer substitutes for the path
- **Universe mandatory** — no frontend ships without a committed world: Design Read (one line — page kind, audience, vibe, archetype line) plus **declared dials** (Density / Variance / Motion, stated, never internal), Concept Spine (one named world that layout, type, color, motion, and copy each express), **anti-default at two altitudes** (reject the lazy default AND the second-most-obvious reach a model told to avoid it would land on), rotation off the previous build's **stylesheet stamp** (palette family / type pairing / hero layout, stated out loud; ≥1 new mechanic per build), signature moment (one loud climax + a quiet second-read detail). A thin, literal, or safe universe is regenerated — and a weak spine caps the whole score via the rubric's **concept veto**
- **Commit-and-prove** — a binding `design_plan` before any markup commits explicit per-element choices (hero architecture, type stack, color roles, the real visual per section, motion paradigms, the signature beat, spacing rhythm) and proves each load-bearing one (the `clamp()`/`max-w` that lands the H1 in ≤2 lines, the named hero asset, the easing + trigger for the signature, the grid spans with zero empty cells). Drifting to a default mid-build is forbidden
- **External truth for heavy layers** — GSAP/ScrollTrigger, Three.js/R3F, Lenis, View Transitions, Web Audio are never written from training memory. Each layer the signature uses resolves through a ladder: installed skill → offer the user the install once (exact command, e.g. the official [gsap-skills](https://github.com/greensock/gsap-skills)) → fetch current docs and cite them. An undeclared source is a pre-flight fail
- **Pre-flight gate** — before ship, the bundled deterministic scanner (`scripts/preflight_scan.py`, Python stdlib, ~20 rules: banned hexes, em-dash density, `h-screen`, scroll listeners, meta-labels, missing alt/landmarks/reduced-motion…) runs first, then every box in `references/preflight.md` ticks — axioms, consistency locks, countable checks with formulas, craft floor, copy audit, browser proof — ending in a filled verdict block. One unticked box blocks "done". The scanner catches, it never clears
...
- **The conformance loop** — browser tooling is a gated capability, not a nice-to-have: Chrome DevTools MCP → `dev-browser` → `webwright`, and when none is present the skill offers the install once (`npm install -g dev-browser && dev-browser install`); only a declined offer degrades to a declared code-level fallback. With tooling, every section loops screenshot → hunt drift against the design_plan and DESIGN.md tokens → fix → re-render, at **375px and 1440px on every iteration** (responsive judged per loop, never retrofitted), exiting only when both widths pass in the same iteration — capped at 5 loops with residue filed in the verdict. The pre-ship pass reads the rendered page, not the markup
- **Composition & optical craft** — the page is paced like a score: per-section intensity committed in the design_plan, exactly one climax (the signature), at least one rest — a flat curve is a template however good each section looks alone. The signature is specified as a **beat table** (trigger / element / transform / duration / ease) in the DESIGN.md before it is coded. `references/optical-craft.md` carries the last 10%: the tracking curve (tighten as size grows), `text-wrap: balance`/`pretty`, tabular numerals, optical centering, surface-tinted borders and shadows, an **interaction-personality map** per archetype (Brutalist snaps in 0–80ms, Corporate Luxury glides in 400–600ms), and a **quiet layer** of ≥2 second-read details (`::selection`, palette favicon, voice-written `<title>`, designed 404). The browser gate also proves the **computed display font resolves to the committed face** — the silent system-font fallback is invisible in code and fatal on screen
- **Award imperatives imposed, not left to taste** — `references/award-imperatives.md` turns the 2025–2030 award formula into gates: a named signature interaction (not a load fade), a real navigation pattern (a show-on-scroll-up header or an overlay — never "no nav"), smooth-scroll narrative, `clip-path` image reveals, micro-interactions, modern CSS (OKLCH / container queries / `:has` / `@property`), a **measured** performance budget (LCP < 1.5s · CLS < 0.05 · INP < 100ms · < 3 MB · 60fps, AVIF/WebP), and mobile reconsidered — each committed in the design_plan and verified at pre-flight, with a per-archetype expression table for how the signature and nav express
- **Comparative review, not absolute** — an adversarial gate grades leniently when it judges "is this nice" in a vacuum; this skill judges the build **against the archetype's canonical winner** (Cartier for Corporate Luxury, Siena for Editorial, Lando Norris for Immersive). The desire read is raised from "would a stranger screenshot it" to "would you be proud to ship this beside the winner, does it beat the current SOTD"; a **premise veto** attacks the concept's idea, not its execution (a coherent metaphor can still manufacture anti-luxury clutter — every literal prop must survive "would a real brand at this tier ship this?"); and the **hero is gated first**, comparatively, before the rest of the page is built
- **Review mode** — `award-design review <url|path>` is the always-on adversarial fresh-eyes: refute by default against the awwwards rubric (Nielsen usability heuristics + the concept and premise vetoes) + the anti-slop catalog + the pre-flight checklist + the award imperatives + the DESIGN.md, with cited fixes and an on-track / off-track verdict, never a silent pass. Runs internally twice per build — R1 refutes the universe before any file, R2 refutes the rendered site before ship — with **anti-anchoring order** (pixels first, scanner second) and, where the harness has subagents, a **multi-lens panel** (comparison-to-winner, restraint, would-you-be-ashamed, rival-mockery) decided by severity and majority rather than a single-reviewer veto
...
- **Anti-AI-slop ambient AND gated** — axiomatic rejections (the AI-purple gradient, Inter/Roboto on the display face, pure `#000`/`#fff`, placeholder names and fake stats, the centered-hero-over-dark template, 3 equal feature cards, `SECTION 01` labels, a hero with no real visual) plus countable archetype-scoped checks, applied ambiently through the build and re-run mechanically at the pre-flight gate
...
- [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill) — the binary-gate / pre-flight-check forcing recipes and the production-tested tell catalog
...
- [greensock/gsap-skills](https://github.com/greensock/gsap-skills) — motion canon, offered for install at the external-truth gate
...
- [Nutlope/hallmark](https://github.com/Nutlope/hallmark) — step-gated pipeline with per-step reference loads and ledger-enforced diversification
- [pbakaus/impeccable](https://github.com/pbakaus/impeccable) — deterministic detector rules and the anti-anchoring dual-assessment review
- [GoogleChrome/modern-web-guidance](https://github.com/GoogleChrome/modern-web-guidance) — the search-first, training-weights-are-stale gate for modern CSS/web APIs
- [alchaincyf/huashu-design](https://github.com/alchaincyf/huashu-design) — hard stop-checkpoints and the concept-veto scoring cap
- [nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill) — auto-appended pre-delivery checklists over deterministic data
