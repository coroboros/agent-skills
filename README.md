...
| Tool | Axis | Source |
| --- | --- | --- |
| `knip` | Simplification (JS/TS dead code) | declared project (node_modules/PnP), else PATH; only under a covering `package.json`, else recorded as not applicable |
...
- **Atomic coverage** — a missing or failed applicable analyzer stops before axis review; fix the printed prerequisite and rerun. An analyzer that cannot apply (Knip with no covering `package.json`) is recorded as not applicable and the review continues.
