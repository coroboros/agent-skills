...
- **Runtime**: bash + Python 3 (stdlib only) for bundled scripts. Skills never resolve packages while they run. Operations backed by external CLIs declare those prerequisites in their SKILL.md and fail with exact install plus rerun guidance when an applicable CLI is unavailable; the org-wide Bun preference does not apply to these portable scripts.
